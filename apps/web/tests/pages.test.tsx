...
describe('management pages', () => {
  ...
  it('shows published Jellyfin setup URLs and copies an endpoint', async () => {
    ...
    expect(screen.getByTestId('endpoint-HDHomeRun tuner URL')).toHaveTextContent('/out/mock-token/hdhr')
    expect(screen.getByTestId('endpoint-HDHomeRun tuner URL')).not.toHaveTextContent('/device.xml')
    await userEvent.click(screen.getByRole('button', { name: 'Copy HDHomeRun tuner URL' }))
    expect(navigator.clipboard.writeText).toHaveBeenCalledWith('http://iptv-web:3000/out/mock-token/hdhr')
